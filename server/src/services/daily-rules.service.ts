...
function calendarHourUTC(d: Date, timeZone: string): number {
...
export function assertGuestVisitWindow(checkIn: Date, checkOut: Date) {
  ...
  if (inH >= 20) {
    throw new Error(
      'Журмын Дэг-3: 20:00 цагаас хойш зочин нэвтрэх боломжгүй (Ховд · Жаргалант — кампусын цагийн бүс)',
    )
  }
}

/** 4.2.11: түр чөлөө 1–3 хоног */
export function assertLeaveDays(leaveDate: Date, returnDate: Date) {
...
export function assertExitNoticeDays(requestedExitDate: Date, now = new Date()) {
