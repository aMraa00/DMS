...
function westGenderToMF(g: unknown): 'M' | 'F' {
  const raw = String(g ?? '').toLowerCase()
  if (raw === 'f' || raw.includes('эмэгтэй') || raw.includes('female')) return 'F'
  return 'M'
}

/** Жишээ: "Б.Амаржаргал" → овогийн товч + нэр */
function splitButenNer(butenNer: unknown): { firstName: string; lastName: string } {
  const t = typeof butenNer === 'string' ? butenNer.trim() : ''
  if (!t) return { firstName: 'Оюутан', lastName: '' }
...
export async function westStudentLogin(
  loginName: string,
  password: string,
  url: string,
  timeoutMs: number,
  ...
  try {
    ...
    if (e instanceof Error && e.name === 'AbortError') {
      ...
      throw new Error(
        `WEST хариу ${sec} сек-оос хэтэрлээ (сүлжээ эсвэл WEST сервер ачаалалтай). Дотоод демо: нэвтрэх хуудасны «И-мэйлээр» таб.`,
      )
    }
    throw e
  } finally {
  ...
  let body: WestLoginResponse = {}
  try {
    body = JSON.parse(text) as WestLoginResponse
  } catch {
    body = { message: text }
  }
  ...
  if (!res.ok) {
    throw new Error(
      typeof body.message === 'string' ? body.message : `WEST login failed (${res.status})`,
    )
  }
...
export function westPayloadToProfile(body: WestLoginResponse): Partial<IUser> & {
  westLoginName: string
} {
  const s = (body.user ?? body.student ?? body.data ?? {}) as Record<string, unknown>
