...
import {
  dateKeyCampusDay,
  dutyAssignedUserForDate,
  syncActiveStudentsOntoDutyRoster,
} from './duty-roster.service.js'

function mongoDuplicateErr(e: unknown): boolean {
...
export async function broadcastInAppNotifications(opts: {
  ...
  const filter =
    opts.audience === 'students'
      ? { role: 'student' as const, status: 'active' }
      : { status: 'active' }
  const users = await User.find(filter).select('_id').lean()
  const recipientCount = users.length
  if (recipientCount === 0) return { recipientCount: 0 }
 ...
 */
export function scheduleContractDeadlineReminders(): void {
 ...
 */
export function scheduleDailyDutyReminders(): void {
  ...
  async function tick() {
    try {
      ...
      await createInAppNotification(
        assigned.userIdStr,
        'Өдрийн жижүүр',
        `${dayKey} өдөр танд жижүүрийн ээлж томилогдсон. Дотуур байрны дотоод журмыг хангаж «Өдөр тутмын» үйлчилгээнд хүлээгдэнэ.`,
        '/daily',
      )
    } catch {
