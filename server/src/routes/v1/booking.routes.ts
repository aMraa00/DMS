...
bookingRouter.get('/slots', async (req: AuthedRequest, res) => {
  const q = z.object({
...
/** Миний захиалгууд */
bookingRouter.get('/my', async (req: AuthedRequest, res) => {
...
bookingRouter.post('/', async (req: AuthedRequest, res) => {
  ...
  void createInAppNotification(
    ...
    `${BOOKING_RESOURCE_LABELS[body.data.resource]} — ${body.data.date} · ${body.data.timeSlot}`,
    '/daily',
  )
