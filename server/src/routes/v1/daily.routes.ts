...
import {
  assertExitNoticeDays,
  assertGuestVisitWindow,
  assertLeaveDays,
} from '../../services/daily-rules.service.js'
...
async function studentHasActiveRoomBooking(userId: string): Promise<boolean> {
...
dailyRouter.get('/duty/my-day', async (req: AuthedRequest, res) => {
...
dailyRouter.post('/guest-passes', async (req: AuthedRequest, res) => {
  ...
  void notifyUsersWithRoles(
    ...
    'Зочны зөвшөөрөл',
    `${parsed.data.guestName} — шинэ зочны бүртгэл ирлээ.`,
    '/admin',
  )
  res.status(201).json({ guestPass: doc })
})

dailyRouter.get('/guest-passes/my', async (req: AuthedRequest, res) => {
...
dailyRouter.post('/leave-requests', async (req: AuthedRequest, res) => {
  ...
  void notifyUsersWithRoles(
    ['admin', 'staff', 'accountant'],
    'Хотоос явах',
    'Оюутан хотоос явах хүсэлт илгээлээ.',
    '/admin',
  )
  res.status(201).json({ leaveRequest: doc })
})

dailyRouter.get('/leave-requests/my', async (req: AuthedRequest, res) => {
...
dailyRouter.post('/exit-requests', async (req: AuthedRequest, res) => {
  ...
  void notifyUsersWithRoles(
    ['admin', 'staff', 'accountant'],
    'Байраас гарах',
    `Шинэ гарах хүсэлт (${new Date(parsed.data.requestedExitDate).toLocaleDateString('mn-MN')}).`,
    '/admin?tab=exits',
  )
  res.status(201).json({ exitRequest: doc })
})

dailyRouter.get('/exit-requests/my', async (req: AuthedRequest, res) => {
...
dailyRouter.post('/complaints', async (req: AuthedRequest, res) => {
  ...
  void notifyUsersWithRoles(
    ['admin', 'staff', 'accountant'],
    isMaintenance ? 'Засварын хүсэлт' : 'Гомдол',
    `[${parsed.data.category}] ${parsed.data.title.slice(0, 80)}`,
    '/admin',
  )
  res.status(201).json({ complaint: doc })
})

dailyRouter.get('/complaints/my', async (req: AuthedRequest, res) => {
...
dailyRouter.post('/room-change-requests', async (req: AuthedRequest, res) => {
  ...
  if (!(await studentHasActiveRoomBooking(uid))) {
    res.status(403).json({
      error:
        'Идэвхтэй өрөөний захиалга (сонгосон өрөө) байхгүй байна. Эхлээд «Өрөө захиалга» цэсээр шинэ захиалга үүсгэнэ үү.',
    })
    return
  }
  ...
  void notifyUsersWithRoles(
    ['admin', 'staff', 'accountant'],
    'Өрөө солих хүсэлт',
    `Шинэ хүсэлт: ${snippet}${parsed.data.reason.length > 120 ? '…' : ''}`,
    '/admin?tab=room-changes',
  )
  res.status(201).json({
    roomChangeRequest: {
      _id: String(doc._id),
...
dailyRouter.get('/room-change-requests/my', async (req: AuthedRequest, res) => {
  ...
  res.json({
    roomChangeRequests: rows.map((r) => {
      let assignedRoom: { dormName: string; roomNumber: number } | undefined
      ...
      if (
        ...
        '_id' in arid &&
        typeof (arid as { roomNumber?: unknown }).roomNumber === 'number'
      ) {
        const room = arid as { roomNumber: number; dormId?: unknown }
        const dz = room.dormId
        const dname =
          dz && typeof dz === 'object' && !(dz instanceof mongoose.Types.ObjectId) && 'name' in dz
            ? String((dz as { name?: unknown }).name ?? '—')
            : '—'
...
dailyRouter.post('/storage-requests', async (req: AuthedRequest, res) => {
  ...
  void notifyUsersWithRoles(
    ['admin', 'staff', 'accountant'],
    'Агуулах',
    'Шинэ агуулахын хүсэлт ирлээ.',
    '/admin',
  )
  res.status(201).json({ storageRequest: doc })
})

dailyRouter.get('/storage-requests/my', async (req: AuthedRequest, res) => {
