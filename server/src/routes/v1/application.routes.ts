...
applicationRouter.post('/', async (req: AuthedRequest, res) => {
  const body = z
    .object({
  ...
  void createInAppNotification(
    String(user._id),
    'Өрөө захиалга',
    'Таны хүсэлт бүртгэгдлээ. Эрэмбийн цонхонд нээлттэй өрөөнөөс сонгоно уу.',
    '/apply',
  )
...
applicationRouter.get('/my', async (req: AuthedRequest, res) => {
...
applicationRouter.get('/priority', async (req: AuthedRequest, res) => {
  const user = await User.findById(req.user!.id)
  if (!user) {
    res.status(404).json({ error: 'User not found' })
    return
...
applicationRouter.put('/:id/select-room', async (req: AuthedRequest, res) => {
  ...
  void createInAppNotification(
    String(user!._id),
    'Төлбөр үүслээ',
    'Өрөө баталгаажлаа. Хугацаа дотор төлбөрөө «Төлбөр» хэсгээс төлж дуусгана уу.',
    '/payments',
  )
...
applicationRouter.post('/:id/pay', async (req: AuthedRequest, res) => {
  const params = z.object({ id: z.string() }).safeParse(req.params)
  const body = z
    .object({
...
function serializeApplication(
  a: InstanceType<typeof RoomApplication>,
  gender?: 'M' | 'F',
) {
  return {
    id: String(a._id),
