...
import { z } from 'zod'
import { authenticate, type AuthedRequest } from '../../middleware/auth.middleware.js'
...
notificationsRouter.get('/', async (req: AuthedRequest, res) => {
  ...
  res.json({
    notifications: notifications.map((n) => {
      const d = n as typeof n & { createdAt?: Date; updatedAt?: Date }
      return {
      _id: String(n._id),
      title: n.title,
      message: n.message,
