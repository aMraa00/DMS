...
function dormGenderAllows(dormGender, studentGender) {
    if (dormGender === 'MIXED')
        return true;
    if (!studentGender)
        return true;
 ...
 */
export async function moveStudentContractToRoom(userId, newRoomId) {
    ...
    if (!contract) {
        return {
            ok: false,
            status: 409,
            message: 'Энэ оюутанд идэвхтэй гэрээ байхгүй тул системд өөрөөр автоматаар шилжүүлэх боломжгүй. Эхлээд гэрээтэй байх ёстой.',
        };
    }
    ...
    if (!dormGenderAllows(dorm.genderType, student?.gender)) {
        return {
            ok: false,
            status: 403,
...
//# sourceMappingURL=move-contract-room.service.js.map