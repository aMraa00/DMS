...
const mockInvoiceFirstCheckAt = new Map();
const MOCK_INVOICE_PAID_AFTER_MS = 12_000;
async function request(method, path, body, authHeader) {
    ...
    if (!res.ok) {
        const p = typeof parsed === 'object' && parsed && parsed !== null
            ? parsed
            : null;
        ...
        const hint = p?.error === 'AUTHENTICATION_FAILED' || res.status === 401
...
async function getToken() {
    const now = Date.now();
    if (_token && now < _tokenExpiry - 60_000)
        return _token;
    if (_refreshToken) {
        try {
            const data = await request('POST', '/v2/auth/refresh', null, `Bearer ${_refreshToken}`);
    ...
    const data = await request('POST', '/v2/auth/token', null, `Basic ${basic}`);
    if (!data.access_token)
        throw new Error('QPay: access_token алга');
    _token = data.access_token;
...
export async function createInvoice(args) {
    ...
    return request('POST', '/v2/invoice', body, `Bearer ${token}`);
}
export async function checkPayment(invoiceId) {
    ...
    const token = await getToken();
    return request('POST', '/v2/payment/check', {
        object_type: 'INVOICE',
        object_id: invoiceId,
