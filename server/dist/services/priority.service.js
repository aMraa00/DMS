 ...
 */
export function computePriorityTier(user) {
    const levelRaw = (user.level || '').toLowerCase();
    const regionRaw = (user.region || '').toLowerCase();
    const isGradOrAdvanced = /\b(doctor|phd|mast|msc|MBA|давтан|эрдмийн|ахисан)\b/i.test(levelRaw + regionRaw);
    const isUbRemote = regionRaw.includes('багахангай') ||
        regionRaw.includes('нарантуул') ||
        regionRaw.includes('алслагдсан');
    const countryside = regionRaw.includes('хөдөө') ||
    ...
    if (user.isDisabled || (countryside && lvlNum === 1))
        return 1;
    if (countryside && lvlNum >= 2 && lvlNum <= 3)
        return 2;
    if (countryside && lvlNum >= 4 && lvlNum <= 5)
        return 3;
    if (isUbRemote)
        return 4;
    if (isGradOrAdvanced)
...
export function priorityWindowEnds(submittedAt) {
