...
async function finalizePaid(payment) {
    ...
    if (payment.applicationId) {
        const app = await RoomApplication.findByIdAndUpdate(payment.applicationId, { status: 'paid' }, { new: true });
    ...
    void createInAppNotification(String(payment.userId), 'Төлбөр амжилттай', 'Төлбөр батлагдлаа. Гэрээнд цахим гарын үсэг зурах шаардлагатай бол «Гэрээ» хэсгийг нээнэ үү.', '/contract');
}
/** QPay webhook (нийтэд) */
export async function qpayCallback(req, res) {
    try {
        ...
        const or = [];
        if (invoiceId)
            or.push({ qpayInvoiceId: invoiceId });
        if (qpayPaymentId)
            or.push({ qpayInvoiceId: qpayPaymentId });
...
export async function createQpayForApplication(req, res) {
    ...
    const host = env.BACKEND_PUBLIC_URL?.replace(/\/$/, '') ?? `${req.protocol}://${req.get('host') ?? 'localhost:4000'}`;
...
export async function createQpayForPendingPayment(req, res) {
...
export async function checkQpayPayment(req, res) {
    ...
    try {
        ...
        const paid = (result?.count ?? 0) > 0 || (Array.isArray(result?.rows) && result.rows.length > 0);
        if (paid) {
            const row = result?.rows?.[0];
            if (row?.payment_id)
                payment.qpayPaymentId = String(row.payment_id);
            await finalizePaid(payment);
...
//# sourceMappingURL=payment-qpay.handlers.js.map