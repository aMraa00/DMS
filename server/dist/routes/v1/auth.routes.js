...
import { authenticate, optionalAccessToken, requireRoles } from '../../middleware/auth.middleware.js';
...
authRouter.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (parsed.data.mode === 'west') {
            const west = await westStudentLogin(parsed.data.loginName, parsed.data.password, env.WEST_LOGIN_URL, env.WEST_FETCH_TIMEOUT_MS);
            const profile = westPayloadToProfile(west);
            userDoc = await User.findOneAndUpdate({ $or: [{ westLoginName: profile.westLoginName }, { studentId: profile.studentId }] }, {
                $set: {
                    ...profile,
                    role: 'student',
                },
...
authRouter.get('/me', optionalAccessToken, async (req, res) => {
    if (!req.user) {
        res.json({ user: null });
        return;
    }
...
authRouter.post('/me/password', authenticate, async (req, res) => {
