...
adminRouter.get('/users', async (req, res) => {
    ...
    const [total, rows] = await Promise.all([
        ...
        User.find(filter)
            ...
            .skip(q.data.skip)
            .limit(q.data.limit)
            .select('email firstName lastName studentId registerNumber phone role status isDisabled westLoginName level region gender school program createdAt updatedAt')
            .lean(),
    ]);
    res.json({
        users: rows.map((row) => serializeAdminUser(row)),
        total,
        skip: q.data.skip,
        limit: q.data.limit,
    });
...
adminRouter.patch('/users/:id', async (req, res) => {
    ...
    res.json({
        ok: true,
        user: fresh
            ? serializeAdminUser(fresh)
            : serializeAdminUser(doc.toObject()),
    });
});
adminRouter.get('/pending-overview', async (_req, res) => {
...
adminRouter.get('/payments/overview', async (req, res) => {
    const q = z
        .object({
...
function serializeExitRow(r) {
    const uid = r.userId;
    const populated = uid && typeof uid === 'object' && '_id' in uid && !(uid instanceof mongoose.Types.ObjectId);
    let userBrief = null;
    if (populated) {
        const u = uid;
        userBrief = {
    ...
    return {
        _id: String(r._id),
        userId: populated ? String(uid._id) : String(r.userId),
        user: userBrief,
...
adminRouter.get('/exit-requests', async (req, res) => {
    const q = z
        .object({
    ...
    res.json({
        exitRequests: rows.map((r) => serializeExitRow(r)),
        pendingCount,
    });
});
adminRouter.post('/exit-requests/:id/approve', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
    if (!params.success || !mongoose.isValidObjectId(params.data.id)) {
        res.status(400).json({ error: 'Invalid id' });
        return;
...
adminRouter.post('/exit-requests/:id/reject', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
...
function serializeRoomChangeRow(r) {
    ...
    const rid = r.assignedRoomId;
    const assignedPopulated = rid && typeof rid === 'object' && '_id' in rid && !(rid instanceof mongoose.Types.ObjectId);
    let assignedBrief;
    if (assignedPopulated) {
        const room = rid;
        const did = room.dormId;
        const d = did && typeof did === 'object' && !(did instanceof mongoose.Types.ObjectId)
            ? did
            : undefined;
        const dormNameRaw = d?.name;
        const rn = typeof room.roomNumber === 'number' ? room.roomNumber : Number(room.roomNumber) || 0;
        assignedBrief = {
            assignedRoomId: String(rid._id),
            assignedRoom: rn > 0
                ? {
...
adminRouter.get('/rooms-for-assignment', async (_req, res) => {
    ...
    res.json({
        rooms: rooms.map((room) => {
            ...
            return {
                ...
                dormName: d?.name ?? '—',
                dormGenderType: d?.genderType ?? 'MIXED',
...
adminRouter.get('/room-change-requests', async (req, res) => {
    const q = z
        .object({
    ...
    res.json({
        roomChangeRequests: rows.map((r) => serializeRoomChangeRow(r)),
        pendingCount,
    });
});
...
adminRouter.post('/room-change-requests/:id/resolve', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
    ...
    await doc.save();
    void createInAppNotification(String(doc.userId), 'Өрөө солих', 'Таны систем дээрх өрөө шинэчлэгдлээ. Лавлагаа хэсгээсээ шалгана уу.', '/room-change');
    ...
    res.json({
        ...
        roomChangeRequest: refreshed
            ? serializeRoomChangeRow(refreshed)
            : null,
    });
});
...
adminRouter.post('/room-change-requests/:id/reject', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
    ...
    doc.reviewedBy = new mongoose.Types.ObjectId(req.user.id);
    await doc.save();
    void createInAppNotification(String(doc.userId), 'Өрөө солих', `Татгалзсан. Тайлбар: ${body.data.note.slice(0, 200)}`, '/room-change');
    const refreshed = await RoomChangeRequest.findById(doc._id)
        .populate('userId', 'studentId registerNumber firstName lastName email')
        .populate({
        path: 'assignedRoomId',
...
adminRouter.post('/exit-requests/bulk-approve', async (req, res) => {
...
adminRouter.post('/exit-requests/bulk-reject', async (req, res) => {
...
adminRouter.post('/room-change-requests/bulk-resolve', async (req, res) => {
...
adminRouter.post('/contracts/create', async (req, res) => {
    if (req.user.role !== 'admin') {
...
adminRouter.get('/violations', async (req, res) => {
    ...
    const filter = {};
    if (q.data.type !== 'all')
        filter.type = q.data.type;
    if (q.data.userId && mongoose.isValidObjectId(q.data.userId))
        filter.userId = q.data.userId;
    const [total, rows] = await Promise.all([
...
adminRouter.post('/violations', async (req, res) => {
    ...
    if (body.data.type === 'WARN') {
        ...
        void createInAppNotification(body.data.userId, `Сануулга #${warningNumber}`, `${body.data.category}: ${body.data.description.slice(0, 200)}`, '/dashboard');
...
adminRouter.get('/violations/:id/warnings', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
    if (!params.success || !mongoose.isValidObjectId(params.data.id)) {
        res.status(400).json({ error: 'Invalid id' });
        return;
...
adminRouter.get('/users/:id/violations', async (req, res) => {
    const params = z.object({ id: z.string() }).safeParse(req.params);
    if (!params.success || !mongoose.isValidObjectId(params.data.id)) {
        res.status(400).json({ error: 'Invalid id' });
        return;
