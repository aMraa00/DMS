...
contractRouter.get('/me', async (req, res) => {
    ...
    res.json({
        contract: {
            ...
            daysLeftToSign: deadlineMs != null ? Math.max(0, Math.ceil((deadlineMs - now.getTime()) / 86_400_000)) : null,
            roomSummary,
        },
    });
});
/** Журмын 3.2.1 — цахим гарын үсэг (PNG/data URL дамжуулах) */
contractRouter.post('/:id/sign', async (req, res) => {
    ...
    await contract.save();
    void createInAppNotification(String(contract.userId), 'Гэрээ баталгаажлаа', 'Таны цахим гарын үсэг хадгалагдлаа.', '/contract');
