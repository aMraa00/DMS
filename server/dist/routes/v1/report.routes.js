...
reportRouter.get('/monthly-summary', async (req, res) => {
    const q = z
        .object({
        yearMonth: z
            .string()
...
reportRouter.get('/export.csv', async (req, res) => {
    const q = z.object({
...
/** Оюутны жагсаалт CSV */
reportRouter.get('/students.csv', async (req, res) => {
    ...
    const rows = students.map((s) => line([s.studentId ?? '', s.lastName ?? '', s.firstName ?? '', s.email ?? '', s.phone ?? '', s.level ?? '', s.region ?? '', s.gender ?? '', s.school ?? '', s.program ?? '', s.status ?? '']));
    const bom = '﻿';
    const csv = [bom + headers, ...rows].join('\n');
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
...
reportRouter.get('/contracts-expiring', async (req, res) => {
